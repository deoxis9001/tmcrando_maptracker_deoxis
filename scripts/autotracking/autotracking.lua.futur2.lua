...
function updateWall(segment, code, address)
	if mystery_seed then
		if items_codes[code]==nil then
			items_codes[code]=false
		end
	else
		if items_codes[code]==nil then
			items_codes[code]=true
		else items_codes[code]==false then
			items_codes[code]=true
		end
	end
	local item = Tracker:FindObjectForCode(code)
	if ReadU8(segment, address) == 0xF3 and items_codes[code] then
		item.Active = true
	elseif ReadU8(segment, address) == 0xF2 and items_codes[code] then
		item.Active = true
	elseif ReadU8(segment, address) == 0xF3 then
		item.Active = false
...
function updateFusion(segment, code, address, flag)
  ...
  if item then
    ...
    if flagTest ~= 0 and items_codes[code] then
      item.Active = true
...
function updateToggleItemFromByteAndFlag(segment, code, address, flag)
  ...
  if item then
    ...
    if TMC_AUTOTRACKER_DEBUG_ITEM then
			print(item.Name, code, flag, items_codes[code])
    end
		if mystery_seed then
			if items_codes[code]==nil then
				items_codes[code]=false
...
function smithswordCheck(segment, code, address, flag)
  if smithsword then
    local value = ReadU8(segment, address)
    if TMC_AUTOTRACKER_DEBUG_ITEM then
      print(smithsword.code, code, flag, items_codes[code])
    end
...
function greenswordCheck(segment, code, address, flag)
  if greensword then
    local value = ReadU8(segment, address)
    if TMC_AUTOTRACKER_DEBUG_ITEM then
      print(greensword.code, code, flag, items_codes[code])
    end
...
function redswordCheck(segment, code, address, flag)
  if redsword then
    local value = ReadU8(segment, address)
    if TMC_AUTOTRACKER_DEBUG_ITEM then
      print(redsword.code, code, flag , items_codes[code])
    end
...
function blueswordCheck(segment, code, address, flag)
  if bluesword then
    local value = ReadU8(segment, address)
    if TMC_AUTOTRACKER_DEBUG_ITEM then
      print(bluesword.code, code, flag, items_codes[code])
    end
...
function fourswordCheck(segment, code, address, flag)
  if foursword then
    local value = ReadU8(segment, address)
    if TMC_AUTOTRACKER_DEBUG_ITEM then
      print(foursword.code, code, flag, items_codes[code])
    end
  ...
  if location then
    ...
    if TMC_AUTOTRACKER_DEBUG_LOCATION then
      print(locationRef, value)
		...
		if mystery_seed then
			if items_codes[locationRef]==nil then
				items_codes[locationRef]=false
			end
		else
			if items_codes[locationRef]==nil then
				items_codes[locationRef]=true
			else items_codes[locationRef]==false then
				items_codes[locationRef]=true
			end
		end

    if (value & flag) ~= 0 and items_codes[locationRef] then
      location.AvailableChestCount = 0
    elseif (value & flag) ~= 0 then
  ...
  if location then
    ...
    if location.Owner.ModifiedByUser then
      return
		...
		if flag==0 and items_codes[locationRef] then
			location.AvailableChestCount = 0
		elseif flag==0 then
			location.AvailableChestCount = 1
		else
 ...
 if location then
  ...
  for i=1, #chestData, 1 do
   ...
   if flagTest ~= 0 then
	...
	if cleared~=0 and items_codes[locationRef] then
...
function updateLLRKey(segment, code, address, flag)
 ...
 if item then
  ...
  if testFlag(segment, address, 0x40) and items_codes[code] then
   item.Active = true
  elseif testFlag(segment, address, 0x80) and items_codes[code] then
   item.Active = true
  elseif testFlag(segment, address, 0x40) then
   item.Active = false
  elseif testFlag(segment, address, 0x80) then
   item.Active = false
  else
   item.Active = false
	 items_codes[code]=true
...
function updateMush(segment, code, address, flag)
 ...
 if item then
  ...
  if testFlag(segment, address, 0x01) and items_codes[code] then
   item.Active = true
  elseif testFlag(segment, address, 0x02) and items_codes[code] then
   item.Active = true
  elseif testFlag(segment, address, 0x01) then
   item.Active = false
  elseif testFlag(segment, address, 0x02) then
   item.Active = false
  else
   item.Active = false
	 items_codes[code]=true

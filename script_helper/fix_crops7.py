...
# Step 3: Pour chaque map_* dans mapspop.json, prendre son location_size comme base
# et écrire size = base * 1.5 dans les entrées "shape":"diamond" des fichiers de location.
MAPS_FILE = os.path.join(BASE_DIR, "maps", "mapspop.json")

with open(MAPS_FILE, "r", encoding="utf-8") as f:
    maps_data = json.load(f)

# Construire un dict { map_name: diamond_size }
map_diamond_sizes = {}
for entry in maps_data:
    name = entry.get("name", "")
    if name.startswith("map_") and "location_size" in entry:
        map_diamond_sizes[name] = math.ceil(entry["location_size"] * 1.4)

print(f"Diamond sizes (×1.5): {map_diamond_sizes}")

# Appliquer dans tous les fichiers de location emo + pop
for prefix in ["emo", "pop"]:
    loc_dir = os.path.join(BASE_DIR, prefix, "json", "locations")
    ...
    for filename in os.listdir(loc_dir):
        ...
        modified = False
        for loc in data:
            for ml in loc.get("map_locations", []):
                if ml.get("shape") == "diamond" and ml.get("map") in map_diamond_sizes:
                    ml["size"] = map_diamond_sizes[ml["map"]]
                    modified = True

        if modified:
            with open(path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            print(f"  Updated diamond sizes in {prefix}/json/locations/{filename}")
